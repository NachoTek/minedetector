...
class TestWinColorPreservation:
        ...
        for row in range(9):
            for col in range(9):
                ...
                if cell.revealed and not cell.mine:
                    ...
                    assert cell.adjacent_mines > 0, "Should have numbers on revealed cells"
            ...
            for col in range(9):
                if not board.grid[row][col].mine:
                    assert board.grid[row][col].revealed, \
                        f"Safe cell ({row},{col}) should be revealed on win"
                    safe_cells_revealed += 1
        
        assert safe_cells_revealed == 71, \
            ...
            for col in range(9):
                ...
                if cell.mine:
                    assert not cell.revealed, \
                        "Mine cells should remain unrevealed"
                    assert not cell.flagged, \
                        "Mine cells should remain unflagged"
        ...
        for row in range(9):
            for col in range(9):
                ...
                if cell.revealed:
                    ...
                    assert not cell.revealed, f"Cell ({row},{col}) should not be revealed"
        ...
        for row in range(9):
            for col in range(9):
                ...
                if not cell.mine:
                    ...
                    assert cell.adjacent_mines > 0, "Revealed safe cells should have numbers"
                    revealed_count += 1
    ...
    def test_large_board_win_color_preservation(self):
        ...
        assert safe_cells_revealed == 381, \
    ...
    def test_color_consistency_amidst_multiple_wins(self):
        ...
        for rows, cols, mines, description in boards:
            ...
            assert revealed_count == expected_safe, \
                f"Should have {expected_safe} safe cells on {description}, got {revealed_count}"
...
class TestHandleGameOverWinColorBehavior:
    ...
    def test_handle_game_over_win_only_modifies_ui_state(self):
        ...
        baseline_game_state = window.board.game_state
...
class TestUpdateCellColorConsistency:
    ...
    def test_update_cell_with_revealed_state(self):
        ...
        assert len(bg_colors) == 1, f"All revealed cells should have same bg, got {bg_colors}"
        assert len(reliefs) == 1, f"All revealed cells should have same relief, got {reliefs}"
    ...
    def test_update_cell_with_unrevealed_state(self):
        ...
        assert len(bg_colors) == 1, f"All unrevealed cells should have same bg, got {bg_colors}"
        assert len(reliefs) == 1, f"All unrevealed cells should have same relief, got {reliefs}"
        
        # Should be raised with lightgray
        assert 'lightgray' in bg_colors, f"Background should be lightgray, got {bg_colors}"
